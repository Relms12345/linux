...
static int ac300_phy_enable(struct phy_device *phydev)
{
	...
	/* Step 2: clk gating */
	val |= AC300_REG00_EPHY_REG_CLK_GATING | AC300_REG00_CLKIN_GATING | AC300_REG00_EFUSE_REG_CLK_GATING;
	ret = mdiobus_write(bus, cfg_addr, 0x00, val);
	if (ret)
		return ret;
	msleep(10);
...
static int ac300_phy_probe(struct phy_device *phydev)
{
	...
	if (IS_ERR(calcell)) {
		if (PTR_ERR(calcell) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		dev_warn(dev, "Failed to get EPHY calibration cell (%pe)\n", calcell);
	} else {
		caldata = nvmem_cell_read(calcell, &callen);
		if (IS_ERR(caldata)) {
			if (PTR_ERR(caldata) == -EPROBE_DEFER)
				return -EPROBE_DEFER;
			dev_warn(dev, "Failed to read EPHY calibration data (%pe)\n", caldata);
		} else {
			if (callen < sizeof(priv->caldata)) {
				dev_warn(dev, "EPHY calibration cell is too short (%zu bytes, expected at least %zu)\n",
					 callen, sizeof(priv->caldata));
				kfree(caldata);
			} else {
				priv->caldata = *caldata;
				kfree(caldata);
				dev_info(dev, "Read AC300 EPHY calibration from nvmem: 0x%04x\n", priv->caldata);
			}
		}
	}
...
{
	/* Bind only if the device tree node specifically declares the AC300 compatible string */
